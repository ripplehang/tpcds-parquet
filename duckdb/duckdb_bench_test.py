...
query_directory = '/opt/mstr/hazheng/duckdb/queries/'
local_parquet_directory = f'/local/path/to/parquet_files/{args.data_size}/'

# Database file and performance CSV file naming
database_file = f'/opt/mstr/hazheng/duckdb/{args.data_size}_{args.source}_{args.object_type}'
creation_performance_csv = f'creation_performance_data_{args.data_size}_{args.source}_{args.object_type}.csv'
query_performance_csv = f'query_performance_data_{args.data_size}_{args.source}_{args.object_type}.csv'
        ...
        if args.source == 'local':
            ...
            for item in os.listdir(local_parquet_directory):
                ...
                if os.path.isdir(item_path):
                    ...
                    con.execute(f"CREATE {args.object_type.upper()} {table_name} AS SELECT * FROM parquet_scan('{item_path}')")
                    end_time = time.time()
